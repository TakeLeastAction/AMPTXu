	...
	do njob=1,njobmax
	   CALL COALdeu(nevent,njob,xpro,xneu,ipro,ineu,xnp,xnn,xnd,xpionp,xpionm,xkaonp,xkaonm,xndv) 
	   rms = 1.76
	   CALL COALhe3(nevent,njob,xpro,xneu,ipro,ineu,rms,xhe3,xnhe3v)
	 ...
	 write(*,*)"he-3/p ratio:",xt/xp,xt2/xp
...
101 format(8(1x,F16.10))
	  ...
	  SUBROUTINE COALdeu(Nevent,njob,xpro,xneu,ipro,ineu,avp,avn,avnd,avpionp,avpionm,avkaonp,avkaonm,xndv)
	  implicit real*8 (a-h,o-z)
	  integer,parameter :: NumPar= 1000, Ntest=1000
	  dimension xpro(8,NumPar,Ntest),xneu(8,NumPar,Ntest)
	  dimension ipro(Ntest),ineu(Ntest),xndv(Ntest)
...
10      continue			
	   ...
	   write(*,*)"d/p ratio: ",avnd/(avp*avn)**0.5
...
101 format((1x,I6),8(1x,F10.5))	  
	  ...
	  SUBROUTINE COALhe3(Nevent,njob,xpro,xneu,ipro,ineu,rms,avt,xntv)
	  implicit real*8 (a-h,o-z)
	  integer,parameter :: NumPar= 1000, Ntest=1000
	  dimension xpro(8,NumPar,Ntest),xneu(8,NumPar,Ntest)
	  dimension ipro(Ntest),ineu(Ntest),xntv(Ntest)
	  character(50) :: datatype
      character(3) :: tit          ! title   
      character(50):: DirP
      datatype='ana/ampt.dat' 
	  ...
	  gc = 1./4.
       do npara=1,Nevent
		...
		xntv(npara) = xnt
		avnt = avnt+xnt
		write(*,*)"Events:",npara,"helium-3 number:",xnt
	   
	   enddo

	   avt = avnt/float(Nevent)
	   avp = avp/float(Nevent)
	   avn = avn/float(Nevent)
